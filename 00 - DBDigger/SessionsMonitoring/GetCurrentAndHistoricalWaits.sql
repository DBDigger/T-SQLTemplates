...
RAISERROR (N'|-Starting Waits collection (wait for 60s)', 10, 1) WITH NOWAIT

DECLARE @minctr DATETIME, @maxctr DATETIME

IF EXISTS (SELECT [object_id] FROM tempdb.sys.objects WHERE [name] LIKE '#tblWaits%')
DROP TABLE #tblWaits
IF NOT EXISTS (SELECT [object_id] FROM tempdb.sys.objects WHERE [name] LIKE '#tblWaits%')
...
IF EXISTS (SELECT [object_id] FROM tempdb.sys.objects WHERE [name] LIKE '#tblFinalWaits%')
DROP TABLE #tblFinalWaits
IF NOT EXISTS (SELECT [object_id] FROM tempdb.sys.objects WHERE [name] LIKE '#tblFinalWaits%')
...
SELECT 'Waits_Last_60s' AS [Information], W1.wait_type, 
